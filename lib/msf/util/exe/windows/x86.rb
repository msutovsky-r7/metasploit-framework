module Msf::Util::EXE::Windows::X86
  ...
  module ClassMethods
    def to_win32pe_vbs(framework, code, opts = {})
      Msf::Util::EXE::Windows::Common.to_exe_vbs(to_win32pe(framework, code, opts), opts)
    end
    
    def to_win32pe_dll(framework, code, opts = {})
        flavor = opts.fetch(:mixed_mode, false) ? 'mixed_mode' : nil
        Msf::Util::EXE::Windows::Common.set_template_default_winpe_dll(opts, ARCH_X86, code.size, flavor: flavor)
        opts[:exe_type] = :dll
    ...
    def to_win32pe(framework, code, opts = {})
      ...
      if opts[:sub_method]
        ...
        # use
        return to_win32pe_exe_sub(framework, code, opts)
      end

      # Allow the user to specify their own EXE template
      Msf::Util::EXE::Common.set_template_default(opts, 'template_x86_windows.exe')

      # Copy the code to a new RWX segment to allow for self-modifying encoders
      payload = Msf::Util::EXE::Windows::Common.win32_rwx_exec(code)
    ...
    def to_winpe_only(framework, code, opts = {}, arch = ARCH_X86)
      # Allow the user to specify their own EXE template
      Msf::Util::EXE::Common.set_template_default(opts, "template_#{arch}_windows.exe")
    ...
    def to_win32pe_old(framework, code, opts = {})
      payload = code.dup
      # Allow the user to specify their own EXE template
      Msf::Util::EXE::Common.set_template_default(opts, 'template_x86_windows_old.exe')
    ...
    def to_win32pe_service(framework, code, opts = {})
      ...
      Msf::Util::EXE::Common.set_template_default(opts, 'template_x86_windows_svc.exe')
    ...
    def to_win32pe_dccw_gdiplus_dll(framework, code, opts = {})
      Msf::Util::EXE::Windows::Common.set_template_default_winpe_dll(opts, ARCH_X86, code.size, flavor: 'dccw_gdiplus')
