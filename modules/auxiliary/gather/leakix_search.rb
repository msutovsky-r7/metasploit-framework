...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize(info = {})
    ...
    register_options([
      ...
      OptString.new('QUERY', [false, 'The LeakIX search query']),
      OptEnum.new('SCOPE', [true, 'Search scope (BULK only supports leak)', 'leak', ['leak', 'service']]),
      OptInt.new('MAXPAGE', [true, 'Max pages to collect (1-500, 20 results/page)', 1]),
      OptInt.new('MAXRESULTS', [false, 'Stop after collecting this many results (0 = unlimited)', 0]),
      OptString.new('TARGET_IP', [false, 'Target IP for HOST action']),
      OptString.new('TARGET_DOMAIN', [false, 'Target domain for DOMAIN/SUBDOMAINS actions']),
